...
Original file is located at
    https://colab.research.google.com/drive/1HmtEMD2adblPzdfmBdMS2qpRtkNZi6S6
"""
...
print("Análisis Estadístico de la Altura:")
print(f"  Media: {np.mean(datos_mexico[:, 0]):.2f} metros")
print(f"  Mediana: {np.median(datos_mexico[:, 0]):.2f} metros")
print(f"  Desviación Estándar: {np.std(datos_mexico[:, 0]):.2f} metros")
print(f"  Mínimo: {np.min(datos_mexico[:, 0]):.2f} metros")
print(f"  Máximo: {np.max(datos_mexico[:, 0]):.2f} metros")
print(f"  Percentil 25: {np.percentile(datos_mexico[:, 0], 25):.2f} metros")
print(f"  Percentil 75: {np.percentile(datos_mexico[:, 0], 75):.2f} metros")

print("\nAnálisis Estadístico de la Cintura:")
print(f"  Media: {np.mean(datos_mexico[:, 1]):.2f} cm")
print(f"  Mediana: {np.median(datos_mexico[:, 1]):.2f} cm")
print(f"  Desviación Estándar: {np.std(datos_mexico[:, 1]):.2f} cm")
print(f"  Mínimo: {np.min(datos_mexico[:, 1]):.2f} cm")
print(f"  Máximo: {np.max(datos_mexico[:, 1]):.2f} cm")
print(f"  Percentil 25: {np.percentile(datos_mexico[:, 1], 25):.2f} cm")
print(f"  Percentil 75: {np.percentile(datos_mexico[:, 1], 75):.2f} cm")

# Histogramas
plt.figure(figsize=(12, 6))

plt.subplot(1, 2, 1)
plt.hist(datos_mexico[:, 0], bins=10, color='blue', edgecolor='black')
plt.xlabel("Altura (metros)")
plt.ylabel("Frecuencia")
plt.title("Distribución de la Altura en México")

plt.subplot(1, 2, 2)
plt.hist(datos_mexico[:, 1], bins=10, color='green', edgecolor='black')
plt.xlabel("Cintura (cm)")
plt.ylabel("Frecuencia")
plt.title("Distribución de la Cintura en México")
